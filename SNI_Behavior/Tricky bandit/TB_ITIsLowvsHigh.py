# -*- coding: utf-8 -*-
"""
Created on Fri Aug  1 20:54:41 2025
...
MAX_ITI = 15                       # 🚫 Drop ITIs longer than this (in seconds)
        ...
        for _, row in block.iterrows():
            if row["Is_Poke"]:
                if last_event_type == "Pellet":
                    iti = (row["Timestamp"] - last_event_time).total_seconds() - last_rt
                    if iti >= 0:
                        success.append({
                            "Mouse": row["Mouse"],
...
#%%
# ========================
# 🔍 Print Per-Mouse Median and Mean ITIs
# ========================
print("\n📈 Median ITI Summary (per mouse):")
summary = []

mice = sorted(set(success_df["Mouse"]) | set(unsuccess_df["Mouse"]))
for mouse in mice:
    row = {"Mouse": mouse}
    for pchance, label in zip([0.25, 0.75], ["Low", "High"]):
        s_cond = success_df[(success_df["Mouse"] == mouse) & (success_df["P_Chance"] == pchance)]
        u_cond = unsuccess_df[(unsuccess_df["Mouse"] == mouse) & (unsuccess_df["P_Chance"] == pchance)]
        row[f"Success_{label}"] = s_cond["ITI"].median() if not s_cond.empty else np.nan
        row[f"Unsuccess_{label}"] = u_cond["ITI"].median() if not u_cond.empty else np.nan
    summary.append(row)

summary_df = pd.DataFrame(summary)
print(summary_df.to_string(index=False))

print("\n📈 Mean ITI Summary (per mouse):")
summary = []
...
for mouse in mice:
    ...
    for pchance, label in zip([0.25, 0.75], ["Low", "High"]):
        s_cond = success_df[(success_df["Mouse"] == mouse) & (success_df["P_Chance"] == pchance)]
        u_cond = unsuccess_df[(unsuccess_df["Mouse"] == mouse) & (unsuccess_df["P_Chance"] == pchance)]
        row[f"Success_{label}"] = s_cond["ITI"].mean() if not s_cond.empty else np.nan
        row[f"Unsuccess_{label}"] = u_cond["ITI"].mean() if not u_cond.empty else np.nan
    summary.append(row)
...
# USER SETTINGS
# ========================
DATA_DIR = r'C:\Users\meagh\OneDrive\Desktop\Tricky bandit\SHAM'  # 🔁 Change to your folder path
TIME_WINDOW = (19, 24)              # ⏰ Time window: 7PM–8PM
BIN_WIDTH = 1                       # 📊 Histogram bin width (seconds)
MAX_ITI = 15                        # 🚫 Drop ITIs longer than this (in seconds)
SAVE_NAME = "combined_iti_histogram"  # 📁 Filename (without extension)
        ...
        for _, row in block.iterrows():
            if row["Is_Poke"]:
                if last_event_type == "Pellet":
                    iti = (row["Timestamp"] - last_event_time).total_seconds() - last_rt
                    if iti >= 0:
                        success.append({"Mouse": row["Mouse"], "P_Chance": row["P_Chance"], "ITI": iti, "Timestamp": row["Timestamp"]})
                elif last_event_type in ["Left", "Right"]:
                    iti = (row["Timestamp"] - last_event_time).total_seconds()
                    if iti >= 0:
                        unsuccess.append({"Mouse": row["Mouse"], "P_Chance": row["P_Chance"], "ITI": iti, "Timestamp": row["Timestamp"]})
                last_event_type = row["Event"]
                last_event_time = row["Timestamp"]
            elif row["Is_Pellet"]:
                last_event_type = "Pellet"
...
def plot_cmfd(success_df):
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig("ITIDistr_Success_SHAM.svg")
    plt.savefig("ITIDistr_Success_SHAM.png")
    plt.show()
