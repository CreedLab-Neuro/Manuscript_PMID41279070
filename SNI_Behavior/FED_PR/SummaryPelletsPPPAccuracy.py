...
for filename in os.listdir(directory):
    if filename.endswith('.csv'):
        ...
        if 'V1' in file_name_without_ext:
            group = 'VEH'
        else:
            group = 'HAL'
...
#df_summary.to_csv('PR_summary_metadata_HaloperidolVehicle.csv', index=False)

print("Summary DataFrame created and saved to 'summary_metadata.csv'.")
#%%
df_analysis = pd.read_csv('PR_summary_metadata_HaloperidolVehicle.csv') ## CHANGE TO WHAT YOU'VE GOT ABOVE

fig, axes = plt.subplots(1, 3, figsize=(18, 6))

sns.barplot(x="Pellets", y = 'Group', data = df_analysis, palette='magma', ax=axes[0])
sns.swarmplot(x="Pellets", y = 'Group', data = df_analysis, hue= 'Sex',ax=axes[0])
axes[0].set_title('Pellets')

sns.barplot(x="PPP1", y = 'Group', data = df_analysis, palette='magma', ax=axes[1])
sns.swarmplot(x="PPP1", y = 'Group', data = df_analysis, hue= 'Sex',ax=axes[1]) 
axes[1].set_title('Pokes per Pellet')

sns.barplot(x="Accuracy", y = 'Group', data = df_analysis, palette='magma', ax=axes[2])
sns.swarmplot(x="Accuracy", y = 'Group', data = df_analysis, hue= 'Sex',ax=axes[2])
axes[2].set_xlim(0.6,1.0)
axes[2].set_title('Accuracy')

plt.savefig('FEDPR_SummaryVEHHAL.svg')
#%%
df_analysis = pd.read_csv('PR_summary_metadata_HaloperidolVehicle.csv')
# Define the model
...
#%%
# Define the model
df_analysis = pd.read_csv(r'C:\Users\meagh\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\FED_PR\FR_summary_metadata_Acute2.csv') ## CHANGE TO WHAT YOU'VE GOT ABOVE
#model = ols('Accuracy ~ C(Sex) * C(Group)', data=df_analysis).fit()
model = ols('PPP2 ~ C(Group)', data=df_analysis).fit()
# Perform the ANOVA
anova_table = sm.stats.anova_lm(model, typ=2)
