...
#directory_naive = (r'C:\Users\meagh\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\FED_PR\Naive')

directory_sham = (r'C:\Users\meagh\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\Haloperidol\rPR_7h\VEH')
directory_sni = (r'C:\Users\meagh\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\Haloperidol\rPR_7h\HAL')
#%%
...
#%%
def calculate_average_histogram(df, bin_edges):
    """Calculate the average histogram for a DataFrame given bin edges."""
...
# Define the bin edges (ensure consistent binning for all datasets)
bin_edges = np.arange(0, 250 + 2, 10)  # Bins from 0 to 250, with a step of 10

# Load the resulting DataFrames
result_df = result_df_naive
result_df1 = result_df_sham
result_df2 = result_df_sni

# Calculate the average histograms
mean_histogram1 = calculate_average_histogram(result_df, bin_edges)
mean_histogram2 = calculate_average_histogram(result_df1, bin_edges)
mean_histogram3 = calculate_average_histogram(result_df2, bin_edges)

# Create figure and axis objects
plt.figure(figsize=(12, 7))

# Plot the average histograms
bar_width = 2  # Smaller bar width for better visualization
plt.bar(bin_edges[:-1] - bar_width, mean_histogram1, width=bar_width, label='Naive', edgecolor='black', alpha=0.5)
plt.bar(bin_edges[:-1], mean_histogram2, width=bar_width, label='Sham', edgecolor='black', alpha=0.5)
plt.bar(bin_edges[:-1] + bar_width, mean_histogram3, width=bar_width, label='SNI', edgecolor='black', alpha=0.5)

# Overlay KDEs
sns.kdeplot(result_df.stack(), bw_adjust=0.5, label='KDE: Naive', linestyle='--', color='black')
sns.kdeplot(result_df1.stack(), bw_adjust=0.5, label='KDE: Sham', linestyle='--', color='blue')
sns.kdeplot(result_df2.stack(), bw_adjust=0.5, label='KDE: SNI', linestyle='--', color='red')
...
#bin_edges3, mean_histogram3 = calculate_average_histogram(result_df3)
...
# Plot the average histograms
plt.bar((bin_edges1[:-1]) - 0.2, mean_histogram1, width=0.5, label='Histogram from SHAM', edgecolor='black', alpha=0.5)
plt.bar((bin_edges2[:-1]) + 0.2, mean_histogram2, width=0.5, label='Histogram from SNI', edgecolor='black', alpha=0.5)
plt.bar((bin_edges3[:-1]) + 0.3, mean_histogram3, width=0.3, label='Histogram from Naive', edgecolor='black', alpha=0.5, color='green')

# Overlay KDEs
sns.kdeplot(result_df.stack(), bw_adjust=0.5, label='KDE for SHAM', linestyle='--', color='blue')
sns.kdeplot(result_df2.stack(), bw_adjust=0.5, label='KDE for SNI', linestyle='--', color='red')
sns.kdeplot(result_df3.stack(), bw_adjust=0.5, label='KDE for Naive', linestyle='--', color='green')
...
plt.grid(True)
#plt.show()
#plt.savefig('HistogramBPelletPrice_ChronicMappedwNaive.svg')
...
plt.grid(True)
plt.savefig('DemandCurve_VEH_mapped.svg')
...
# Plot the average raw frequency distribution with error bars (SEM)
plt.errorbar(average_price_counts.index, average_price_counts.values, yerr=sem_price_counts, 
             fmt='o', color='red', ecolor='red', elinewidth=1.5, capsize=3, label='Average with SEM')
...
#plt.show()
plt.savefig('DemandCurve_HAL_V2mapped.svg')
...
# Get the number of subjects (columns)
num_subjects = df3.shape[1]

# Loop through each subject (each column in the DataFrame)
for col in df3.columns:
    # Drop NaN values if any exist
    prices = df3[col].dropna()
    
    # Count the frequency of each unique price for this subject
    price_counts = prices.value_counts().sort_index()  # Sort by price for plotting
    
    # Plot the raw frequency distribution for the subject (Pellet Count on Y-axis, Price Paid on X-axis)
    plt.plot(price_counts.index, price_counts.values, color='black', alpha=0.3, linewidth=1)
...
# Show the plot (no legend)
#plt.show()
plt.savefig('DemandCurve_Naive_Acute.svg')