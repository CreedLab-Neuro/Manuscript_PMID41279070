...
directory = (r'C:\Users\meaghan.creed\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\Metformin\MetforminPR\Cropped')
...
for filename in os.listdir(directory):
    if filename.endswith('.csv'):
        ...
        if '_Met_' in file_name_without_ext:
            group = 'MET'
        elif '_Veh_' in file_name_without_ext:
            group = 'VEH'
        else:
            group = 'Oops'
...
df_summary.to_csv('PR_summary_metadata_MetforminSNI.csv', index=False)

print("Summary DataFrame created and saved to 'summary_metadata.csv'.")
#%%
df_analysis = pd.read_csv('PR_summary_metadata_MetforminSNI.csv') ## CHANGE TO WHAT YOU'VE GOT ABOVE
...
plt.savefig('PR_summary_metadata_MetforminSNI.svg')
#%%
df_analysis = pd.read_csv('PR_summary_metadata_MetforminSNI.csv')
# Define the model
...
print("Summary 2 way ANOVA for Accuracy rPR Pellets")
print(anova_table)
