...
#directory_veh = (r'C:\Users\meagh\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\Metformin\MetforminPR\Cropped\Crop_SNI_VEH')
#directory_met = (r'C:\Users\meagh\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\Metformin\MetforminPR\Cropped\Crop_SNI_MET')
#directory_veh = (r'C:\Users\meagh\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\Metformin\MetforminPR\Cropped\Crop_SHAM')

directory_veh = (r'C:\Users\meagh\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\FED_ASP\Cropped\Pre')
directory_hal = (r'C:\Users\meagh\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\FED_ASP\Cropped\Post')
...
for filename in os.listdir(directory_veh):
    if filename.endswith('.csv'):
        filepath = os.path.join(directory_veh, filename)
        # Read the csv file
...
result_df_veh = pd.concat(data_frames, axis=1)
#%%
# List to hold the series for each csv file in the SHAM directory
data_frames = []

# Iterate over each file in the directory
for filename in os.listdir(directory_hal):
    if filename.endswith('.csv'):
        filepath = os.path.join(directory_hal, filename)
        # Read the csv file
        df = pd.read_csv(filepath)
...
# Combine all series into a DataFrame
result_df_hal = pd.concat(data_frames, axis=1)
...
result_df = result_df_veh #pd.read_csv('result.csv')
result_df2 = result_df_hal #pd.read_csv('result2.csv')  # Assuming 'result2.csv' is your second DataFrame
...
# Load the resulting DataFrames
result_df = result_df_veh.replace(mapping) #pd.read_csv('result.csv')
#result_df2 = result_df_hal.replace(mapping) #pd.read_csv('result2.csv')  # Assuming 'result2.csv' is your second DataFrame
...
plt.grid(True)
#plt.show()
#plt.savefig('HistogramBPelletPrice_MetSHAMs.svg')
...
plt.grid(True)
#plt.savefig('DemandCurve_Met_VEH.svg')
...
for col in df2.columns:
    ...
    # Plot the raw frequency distribution for the subject (Pellet Count on Y-axis, Price Paid on X-axis)
    plt.plot(price_counts.index, price_counts.values, color='green', alpha=0.3, linewidth=1)
...
#plt.show()
plt.savefig('DemandCurve_PostASP.svg')