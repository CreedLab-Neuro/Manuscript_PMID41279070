...
directory = (r'C:\Users\meaghan.creed\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\FED_ASP\Cropped')
...
for filename in os.listdir(directory):
    if filename.endswith('.csv'):
        ...
        if 'Male' in file_name_without_ext:
            sex = 'M'
        elif 'Female' in file_name_without_ext:
            sex = 'F'
        else:
            sex = 'Uknown'
        
        # Determine 'Group' based on file name
        if 'Pre' in file_name_without_ext:
            group = 'Pre'
        elif 'Post' in file_name_without_ext:
            group = 'Post'
        else:
            group = 'Oops'
...
df_summary.to_csv('PR_summary_metadata_ASP.csv', index=False)

print("Summary DataFrame created and saved to 'PR_summary_metadata_ASP.csv'.")
#%%
df_analysis = pd.read_csv(r'C:\Users\meaghan.creed\Box\CreedLabBoxDrive\Jeremy\SNI_Dopamine_Paper\Figure1_SNIBehavior\FED_ASP\PR_summary_metadata_ASP.csv') ## CHANGE TO WHAT YOU'VE GOT ABOVE
...
#plt.savefig('PR_summary_metadata_ASP.svg')
#%%
#df_analysis = pd.read_csv('PR_summary_metadata_ASP.csv')
# Define the model
...
print("Summary 2 way ANOVA for rPR Accuracy")
print(anova_table)
...
#%%
# Define the model
df_analysis = pd.read_csv('FR_summary_metadata_ChronicWithNaive.csv') ## CHANGE TO WHAT YOU'VE GOT ABOVE
model = ols('Pellets ~ C(Sex) * C(Group)', data=df_analysis).fit()

# Perform the ANOVA
anova_table = sm.stats.anova_lm(model, typ=2)

# Print the ANOVA table
print("Summary 2 way ANOVA for Pellets")
print(anova_table)

# Group the data by 'Group' and 'Sex', then calculate the mean and SEM for each group
grouped = df_analysis.groupby(['Group', 'Sex'])['Pellets']
